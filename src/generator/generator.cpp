...
getRandomCell(std::shared_ptr<Region> region, CellSpec type) {
  if (region->rooms.size() == 0) {
    return std::nullopt;
  }

  auto room = region->rooms[rand() % region->rooms.size()];
  auto rc = room->getRandomCell(type);
  auto n = region->rooms.size();
  while (!rc && n > 0) {
    room = region->rooms[rand() % region->rooms.size()];
    auto rc = room->getRandomCell(type);
    n--;
  }
  return rc;
...
void placeInRoom(std::shared_ptr<Region> region, std::shared_ptr<Room> room) {
  ...
  region->rooms.push_back(room);
}

void placeStartInRoom(std::shared_ptr<Region> region, std::shared_ptr<Room> room) {
  auto floor = region->type.floor;
  auto ltcr = *getRandomCell(location, floor);
...
void placeRoom(std::shared_ptr<Region> region, std::shared_ptr<Room> room) {
...
void placeWalls(std::shared_ptr<Region> region) {
  ...
  for (auto cell : region->cells.front()) {
    if (cell->type != CellType::UNKNOWN) {
      cell->type = CellType::WALL;
      cell->seeThrough = false;
      cell->passThrough = false;
  ...
  for (auto cell : region->cells.back()) {
    if (cell->type != CellType::UNKNOWN) {
      cell->type = CellType::WALL;
      cell->seeThrough = false;
      cell->passThrough = false;
...
void makePassageBetweenRooms(std::shared_ptr<Region> region,
  ...
  region->rooms.push_back(passage);
...
void fixOverlapped(std::shared_ptr<Region> region) {
  ...
  for (auto r : region->rooms) {
    cache[r] = {};
  }
  for (auto r = 0; r < HEIGHT; r++) {
    for (auto c = 0; c < WIDTH; c++) {
      if (region->cells[r][c]->type == CellType::UNKNOWN) {
        continue;
      }
      std::vector<std::shared_ptr<Room>> rooms(region->rooms.size());
      auto it = std::copy_if(region->rooms.begin(), region->rooms.end(),
      ...
      if (rooms.size() == 1) {
        cache[rooms.front()].push_back(region->cells[r][c]);
        region->cells[r][c]->room = rooms.front();
        ...
        if (halls.size() == 0) {
          ...
          cache[halls.front()].push_back(region->cells[r][c]);
          region->cells[r][c]->room = halls.front();
        } else {
  ...
  auto rooms = region->rooms;
  for (auto room : rooms) {
    // FIXME: remove orphaned objects
    room->cells = cache[room];
    if (room->cells.size() == 0) {
      region->rooms.erase(
          std::remove(region->rooms.begin(), region->rooms.end(), room));
    }
  }
}
  ...
  for (auto r : region->cells) {
    for (auto c : r) {
      if (c->type == CellType::FLOOR) {
        if ((region->cells[c->y - 1][c->x]->type == CellType::WALL &&
             region->cells[c->y + 1][c->x]->type == CellType::WALL) ||
            (region->cells[c->y][c->x - 1]->type == CellType::WALL &&
             region->cells[c->y][c->x + 1]->type == CellType::WALL)) {
...
void placeEnemies(std::shared_ptr<Region> region, int threat) {
  ...
  if (region->type.type == LocationType::DUNGEON) {
  ...
  for (auto room : region->rooms) {
    ...
    auto it =
        std::copy_if(room->cells.begin(), room->cells.end(), cells.begin(),
                     [&location](auto cell) { return cell->type == region->type.floor; });
...
void makeRiver(std::shared_ptr<Region> region) {
  dlog("place river");
  auto river =
      dig(region->cells[0][region->cells.front().size() / 3 +
                             rand() % region->cells.front().size() / 3],
          S, region->cells.size() - 2, 2, 2, 1, CellType::WATER);
...
bool placeStairs(std::shared_ptr<Region> region) {
  ...
  if (region->type.enterCell != nullptr) {
    ...
    entr =
        std::make_optional<std::shared_ptr<Cell>>(region->cells[e->y][e->x]);
  }
  dlog("entr found 1");
...
void makePassages(std::shared_ptr<Region> region) {
  ...
  std::vector<std::shared_ptr<Room>> halls = region->rooms;

  while (n < pc) {
    auto room1 = region->rooms[rand() % region->rooms.size()];
    auto room2 = region->rooms[rand() % region->rooms.size()];
    if (room1 == room2)
      continue;
  ...
  for (auto r : halls) {
    if (std::find(rooms.begin(), rooms.end(), r) == rooms.end()) {
      auto room2 = region->rooms[rand() % region->rooms.size()];
      while (room2 == r) {
        room2 = region->rooms[rand() % region->rooms.size()];
      }
...
void Generator::makeExterior(std::shared_ptr<Region> region) {
  ...
  region->cells = mapUtils::fill(HEIGHT*2, WIDTH*2, CellType::UNKNOWN);
  for (auto n = 0; n < rc; n++) {
    ...
    auto ry = rand() % (region->cells.size() - room->height - 8) + 8;
    auto rx = rand() % (region->cells.front().size() - room->width - 8) + 8;
      ...
      for (auto c : r) {
        if (c->type == CellType::UNKNOWN) {
          ...
          if ((fn >= 4 && rand() % 10 < 8) || fn == 3) {
            // c->type = CellType::GROUND;
            mapUtils::updateCell(region->cells[c->y][c->x], CellType::GROUND, c->features);
          }
        } else {
          auto n = region->getNeighbors(c);
          auto fn =
              ...
              if (fn >= 4) {
                ...
                // mapUtils::updateCell(region->cells[y][x], CellType::UNKNOWN, c->features);
              }
...
void placeRooms(std::shared_ptr<Region> region) {
  ...
  for (auto n = 0; n < rc; n++) {
    ...
    auto ry = rand() % (region->cells.size() - room->height - 2) + 2;
    auto rx = rand() % (region->cells.front().size() - room->width - 2) + 2;
    mapUtils::paste(room->cells, location, rx, ry);
    room->x = rx;
    room->y = ry;
    room->threat = rand() % 4;
...
void Generator::makeCavePassage(std::shared_ptr<Region> region) {
  ...
  for (auto n = 0; n < rand() % 6 + 2; n++) {
    ...
    region->rooms.push_back(newRoom);
...
void placeTemplateInRoom(std::shared_ptr<Region> region, std::shared_ptr<RoomTemplate> rtp) {
  auto room = rtp->generate(location);
  // Room::printRoom(location, room);

  //TODO: shuffle suitable rooms and select
  auto target = region->rooms[rand() % region->rooms.size()];
  auto n = 0;
  while (target->width - 2 < room->width || target->height -2 < room->height || target->x == 0) {
    target = region->rooms[rand() % region->rooms.size()];
    if (n > 30) {
...
void placeTemplate(std::shared_ptr<Region> region, std::shared_ptr<RoomTemplate> rtp) {
  auto room = rtp->generate(location);
  placeRoom(location, room);
}

void placeStartTemplateInRoom(std::shared_ptr<Region> region, std::shared_ptr<RoomTemplate> rtp) {
  auto room = rtp->generate(location);
...
std::shared_ptr<Room> placeVoid(std::shared_ptr<Region> region) {
...
std::shared_ptr<Room> placeLake(std::shared_ptr<Region> region) {
...
std::shared_ptr<Location> Generator::getLocation(LocationSpec spec) {
  ...
  if (spec.type == LocationType::DUNGEON) {
    ...
    if (region->hasFeature(LocationFeature::CAVE_PASSAGE)) {
      start = std::chrono::system_clock::now();
  ...
  if (region->hasFeature(LocationFeature::RIVER)) {
    start = std::chrono::system_clock::now();
    makeRiver(location);
    end = std::chrono::system_clock::now();
  ...
  if (region->hasFeature(LocationFeature::VOID)) {
    start = std::chrono::system_clock::now();
    placeVoid(location);
    end = std::chrono::system_clock::now();
  ...
  if (region->hasFeature(LocationFeature::LAKE)) {
    start = std::chrono::system_clock::now();
    placeLake(location);
    end = std::chrono::system_clock::now();
  ...
  if (region->hasFeature(LocationFeature::ALTAR)) {
    start = std::chrono::system_clock::now();
    placeAltar(location);
    end = std::chrono::system_clock::now();
  ...
  if (region->hasFeature(LocationFeature::TORCHES)) {
    start = std::chrono::system_clock::now();
  ...
  if (region->hasFeature(LocationFeature::STATUE)) {
    start = std::chrono::system_clock::now();
  ...
  if (region->hasFeature(LocationFeature::TREASURE_SMALL)) {
  ...
  for (auto r : region->cells) {
    for (auto c : r) {
      ...
      } else if (c->room != nullptr && c->room->type == RoomType::HALL && c->type == region->type.floor && R::R() < data->probability["CRATE"]) {
        region->addTerrain("CRATE", c);
